...
void edit()
{
    ifstream fp1;
    ofstream fp2;
    fp1.open("D:/QT project/Compile/compile/error.txt");
    fp2.open("D:/QT project/Compile/compile/in.txt");
...
bool redef_or_notdef()
{
    ...
    for (int i = 1; i < all.size(); i++)
    {
        ...
        if (all[i][0].value == "int" || all[i][0].value == "char" || all[i][0].value == "float" || all[i][0].value == "string" || all[i][0].value == "struct")
        {
            ...
            {
                getcout << "重定义标识符：" << all[i][1].value << endl;
                flag = false;
            }
        }
        for (; j < all[i].size(); j++)
        {
            if (all[i][j].type == 'B')
            {
                if (teststack.find(all[i][j].value) != teststack.end())
                    continue;
                getcout << "未定义标识符：" << all[i][j].value << endl;
                flag = false;
            }
        }
    }
...
void readin()
{
    ...
    Getch[0] = "#";
    ifstream fp;
    fp.open("D:/QT project/Compile/compile/zhongjie.txt");
    if (!fp.is_open())
    {
        getcout << "error3" << endl;
        errorFlag = true;
    ...
    Getnu["@"] = numvt;
    Getch[num] = ("@");
    fp.open("D:/QT project/Compile/compile/nozhongjie.txt");
    if (!fp.is_open())
    {
        getcout << "error4" << endl;
        errorFlag = true;
    ...
    fp.close();
    fp.open("D:/QT project/Compile/compile/chanshengshi.txt");
    if (!fp.is_open())
    {
        getcout << "error5" << endl;
        errorFlag = true;
...
void Printfirst() //打印first集
{
    ...
    for (int i = 1; i <= num; i++)
    {
        getcout << "first [" << Getch[i] << "]: ";
        for (int j = 0; j < first[i].size(); j++)
            getcout << Getch[first[i][j] - '0'] << " ";
        getcout << endl;
    }
    getcout << endl;
}
...
{
    Intersection(follow[Getnu[Gainfirst(Process.front())]], "0"); //首个加入#
    for (int j = 0; j < Process.size(); j++)
    {
        int jj = 0;
        while (Process[j][jj++] != ' ')
...
{
    getcout << "follow集：" << endl;
    for (int i = numvt + 1; i <= num; i++)
    {
        getcout << "follow [" << Getch[i] << "]: ";
        for (int j = 0; j < follow[i].size(); j++)
            getcout << Getch[follow[i][j] - '0'] << " ";
        getcout << endl;
    }
    getcout << endl;
}
...
void PrintT()
{
    ...
    string ss;
    int t;
    getcout << left << setw(12) << " ";
    for (int i = 0; i < numvt; i++)
    {
        ss = Getch[i];
        t = ss.size();
        getcout << left << setw(t + 2) << ss;
    }
    getcout << endl;
    for (int i = numvt + 1; i <= num; i++)
    {
        getcout << left << setw(12) << Getch[i];
        for (int j = 0; j < numvt; j++)
        {
            ss = Getch[j];
            t = ss.size();
            getcout << left << setw(t + 2) << Getp(table[i][j]);
        }
        getcout << endl;
    }
    getcout << endl;
...
{
    ifstream file;
    char filename[] = "D:/QT project/Compile/compile/chanshengshi1.txt";
    file.open(filename, ios::in);
    if (!file.is_open())
        return;
    string strline;
...
void printfour()
{
    ...
    {
        getcout << left << setw(10) << chararray[i].name1.value << left << setw(10) << chararray[i].name2.value << left << setw(10) << chararray[i].name3.value << left << setw(10) << chararray[i].name4.value << endl;
    }
}
void semantic_analysis()
{
    ...
    while (num < ALL.size())
    {
        ...
        semantictoken.pop();
        getcout << str1 << "  " << ALL[num].value << endl;
...
void printfnewfour() //打印
{
    ...
    {
        getcout << left << setw(10) << newchararrray[k].name1.value;
        getcout << left << setw(10) << newchararrray[k].name2.value;
        getcout << left << setw(10) << newchararrray[k].name3.value;
        getcout << left << setw(10) << newchararrray[k].name4.value << endl;
    }
}
void insertfour()
{
...
void printchart()
{
    getcout << endl
         << "符号表如下" << endl;
    getcout << left << setw(10) << "name" << left << setw(10) << "type" << left << setw(10) << "cat" << left << setw(10) << "address" << endl;
    for (int k = 0; k < mainchart1[0].length; k++)
    {
        getcout << left << setw(10) << mainchart1[k].name;
        getcout << left << setw(10) << mainchart1[k].type;
        getcout << left << setw(10) << mainchart1[k].cat;
        getcout << left << setw(2) << mainchart1[k].address.point << ' ' << mainchart1[k].address.type << endl;
    }
    // getcout << "函数表如下" << endl;
    // getcout << left << setw(10) << "level" << left << setw(10) << "off" << left << setw(10) << "fn" << endl;
    // for (int k = 0; k < funcchart1[0].length; k++)
    // {
    //     getcout << left << setw(10) << funcchart1[k].level;
    //     getcout << left << setw(10) << funcchart1[k].off;
    //     getcout << left << setw(10) << funcchart1[k].fn << endl;
    //     getcout << left << setw(10) << "name" << left << setw(10) << "type" << left << setw(10) << "cat" << left << setw(10) << "address" << endl;
    //     for (int j = 0; j < funcchart1[k].para[0].length; j++)
    //     {
    //         getcout << left << setw(10) << funcchart1[k].para[j].name;
    //         getcout << left << setw(10) << funcchart1[k].para[j].type;
    //         getcout << left << setw(10) << funcchart1[k].para[j].cat;
    //         getcout << left << setw(2) << funcchart1[k].para[j].address.point << ' ' << funcchart1[k].para[j].address.type << endl;
    //     }
    // }
    // getcout << "类型表如下" << endl;
    // getcout << left << setw(10) << "type" << left << setw(10) << "address" << endl;
    // for (int k = 0; k < typechart1.size(); k++)
    // {
    //     getcout << left << setw(10) << typechart1[k].type << typechart1[k].address.point << ' ' << typechart1[k].address.type << endl;
    // }
    // getcout << "结构体表如下" << endl;
    // getcout << left << setw(10) << "name" << left << setw(10) << "off" << left << setw(10) << "type" << endl;
    // for (int k = 0; k < structchart1.size(); k++)
    // {
    //     getcout << left << setw(10) << structchart1[k].name << left << setw(10) << structchart1[k].off << left << setw(10) << structchart1[k].type << endl;
    // }
    // getcout << "长度表" << endl;
    // for (int k = 0; k < length.size(); k++)
    // {
    //     getcout << length[k] << endl;
    // }
    // getcout << "活动记录如下" << endl;
    // getcout << left << setw(10) << "name" << left << setw(10) << "type" << left << setw(10) << "low" << left << setw(10) << "up" << endl;
    // for (int k = 0; k < vall1[0].length; k++)
    // {
    //     getcout << left << setw(10) << vall1[k].name;
    //     getcout << left << setw(10) << vall1[k].type;
    //     getcout << left << setw(10) << vall1[k].low;
    //     getcout << left << setw(10) << vall1[k].up << endl;
    // }
}
...
void CreateTarget()
{
    char target[] = "D:/QT project/Compile/compile/final.asm";
    int w = 0;
    int w1 = 3;
    int w2 = 3;
...
int compilemain()
{
    char filepath[] = "D:/QT project/Compile/compile/in.txt";
    getcout.open("D:/QT project/Compile/compile/log.txt");
    edit();
    if(errorFlag == true)
            return 100;
    autoinit();
